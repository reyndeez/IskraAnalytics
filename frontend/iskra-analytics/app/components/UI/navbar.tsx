...
export default function Navbar() {
    ...
    const tabsByRole: Record<any, {label: string; href: string}[]> = {
    coach: [
    ...
    const userTabs = (user  !== null) ? tabsByRole[user.role as keyof typeof tabsByRole] : [];
            ...
            <div className="py-6 flex justify-between items-center gap-2 px-[5%] md:px-[10%]">
                        ...
                        return (
                            <Link 
                                ...
                                className={`
                                    ...
                                    ${isActive 
                                        ? 'text-brand' 
                                        : 'text-[#808080] hover:text-brand hover:bg-blue-50/40'
                                    }
                                `}
                            >
                                {tab.label}
                                ...
                                {isActive && (
                                    <span className="absolute top-19 bottom-0 left-0 right-0 h-1.5 bg-brand rounded-t-full" />
                                )}
                            </Link>
                        );
                    })}
                ...
                <div className="flex gap-8 items-center font-medium">
                    {!user && isHome && (
                        <>
                        <button 
                        className="text-brand cursor-pointer  text-2xl hover:text-[#41479B] border border-transparent px-4 py-2 hover:border-[#41479B] hover:rounded-xl transition-all duration-200"
                        onClick={() => setAuthMode('register')}
                        >
                            Зарегистрироваться
                        </button>
                        <button 
                        className="px-4 py-2 bg-brand cursor-pointer rounded-xl text-white  text-2xl hover:bg-[#41479B] shadow-md hover:shadow-lg" 
