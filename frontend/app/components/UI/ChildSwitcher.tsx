...
export default function ChildSwitcher({ childrenList }: ChildSwitcherProps) {
    ...
    const activeId = searchParams.get('studentId');

    useEffect(() => {
        const savedId = localStorage.getItem('selectedStudentId');
        
        if (!activeId && savedId && childrenList.some(c => c.id.toString() === savedId)) {
            const params = new URLSearchParams(searchParams.toString());
            params.set('studentId', savedId);
            router.replace(`?${params.toString()}`, { scroll: false });
        }
    }, [activeId, childrenList, router, searchParams]);
    ...
    return (
        <div className="flex items-center gap-3 bg-white/50 backdrop-blur-md p-2 rounded-full shadow-inner border border-white">
            {childrenList.map((child: Child) => (
                <button
                    ...
                    className={`cursor-pointer w-12 h-12 rounded-full flex items-center justify-center text-white font-bold transition-all duration-300
                        ${activeId === child.id.toString() 
                            ? 'bg-[#064592] ring-4 ring-blue-100 scale-105 shadow-md' 
            ...
            <button 
                ...
                className="cursor-pointer w-12 h-12 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 hover:border-[#064592] hover:text-[#064592] transition-colors"
            >
